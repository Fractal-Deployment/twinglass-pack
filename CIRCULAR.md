# Circular construct around V
**Class:** WORKING  
**Bound:** 2026-09-15 NS-to-LLMVE cut  
This file is **additive**. It does not replace `RING.md`, `APPARATUS.md`, or `WORKFLOW_HOST.md`.
It is **not** a second ten-pod roster. `CAP: 10` still counts **live walkers**, not sleeping packets.

The NS piece is real. It is not “ten standing teams, one named Exergy.”
Functionality of NS is still in the machine. The **10 chairs** named Exergy / Mythos / … are not.

Do not copy LLMVE meters. Court pointer stays `Fractal-Deployment/llmve-meaning` `VERSION`. Drive copies are past.

## What NS actually ran
Not 80. Published shape:

- Warm-up on a **cousin** (unforced Euler): ~100 agents.
- Then NS at ~**10,000** concurrent.
- Groups isolated. Different groups got **variants of the same problem**.
- Clay split: A/B aimed at a **smoothness proof**, C/D aimed at a **disproof**. Separate groups. Exclusive accounts, not one committee.
- Codex **consolidated markers** from intermediates and wrote the next prompts.
- Formalize **after** search (~17h Lean). They are not claiming the prize.

That is the methodology that produced the outcome: cousin first, variant split, proof-side vs disproof-side as **exclusive tracks**, consolidator, bank failures, verify last. Volume was how they searched, not why the ideas worked.

## What we copy
Refuse **10,000 as the method**. Keep:
...
Operator ask that morning: **10 pods + rewriter**, not 10k. Then the ten **standing** pods were cut (below).

Proof/disproof here is **not** two political camps. It is: “this expression covers the word problem” vs “this expression is the wrong object.”
P9 was one **disconfirm** pod. Proof-side vs disproof-side was also written *inside* each pod.
Rewriter (DLG) = their Codex: 3-ply lookahead, markers only, telos unmoved.

## What we cut
Ten **standing** pods — one crew owns Exergy, one owns Mythos — **refused**.

`necessary_because`: a seated owner defends the name and invents-green. That repeats NS “one variant only” at N=8.

## Bound form (still in the machine)

| NS piece | Bound form now |
|---|---|
| 10k volume | refuse |
| 10 at a time | **cap 10 = STOP**, not a roster to fill |
| Proof group vs disproof group | **twin** only if two accounts cannot both be true. Default is **one walker** + `negation_map[]` |
...
| Circle | ring **schedule**: one `LIVE_NAME`, sleep packet, wake on neighbor quotes |

Width 10 is a ceiling **if spawn quotes exist**. Default live width is **1** (or **2** only as Energy P vs R inside **one** packet).

Everyday walk is thesis + negation-map on one pad, then the ring pushes the neighbor.
The NS exclusive split is the **twin door**, leftover only. Default gait is not “Proof team vs Disproof team.” That would be dialectic, already thrown out.
...
## Seats
A seat is a **locked name**. Status: `unnamed` | `live` | `sleeping`.
The name is not owned by a standing team. Disk holds hibernated packets.

| Language | Seats |
|---|---|
| LLMVE project charge | Energy, Path, Mythos — as **names on the ring**, not owner folders |
| General | Names as they lock (event-plane, G, energy form, path, units). **Do not mint mythos tags.** |

## Push-pull
A live name walks until the construct **butts against** a neighbor it cannot carry. That is the Sleep door **plus** a wake quote for the neighbor.
...
live Energy
  ...
  LIVE_NAME := Path
live Path
  cannot carry remainder / C_d face
  Sleep Path
