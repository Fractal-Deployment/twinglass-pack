...
| Monitor | `lcd-glossary-integrity` + `lcd-lens` | Terms (SI) and function-sets (LCD). Two objects. Laws are **method**, not a third object. |
| Rewrite | `honest-prompt-rewrite` | Working prompt under LRR. |
| Meaning | `llmve-meaning` | What a meter may answer. Pointer to 0.3.4. |
| Values | `reason-telos-lookup` | Hierarchy, not preference matrix. |
...
inventory → per-skill LCD (collection vs demand)
         ...
         → diamond one skill at a time if a restatement is earned
         → collate-hourglass
...
**Do not** load twin. **Do not** mint. **Do not** run factor-compute. **Do not** implement Conley/MDL as code.
...
|---|---|
| `skill` | folder name |
| `collection` | what the files actually do (quote paths) |
| `claimed` | README / frontmatter job |
...
LCD: collection ∩ demand. If empty, **look elsewhere** — don’t convert a metaphor into a meet.
...
- Prior CLI boards (`PACES_BOARD`, what-lake synthesis, isolation cascade rank 0 on **new** `grok` only). Historical. Don’t mint from them.
