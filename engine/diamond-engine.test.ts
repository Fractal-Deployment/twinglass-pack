...
function fourFinished(run: DiamondRun): DiamondRun {
  let r = run;
...
test("cannot mark before all four pads are finished", () => {
...
test("mark is a restatement, not a refusal", () => {
  const run = fourFinished(openRun({ charge: CHARGE, walker: "single" }));
...
test("empty pad cannot finish", () => {
  const run = openRun({ charge: CHARGE, walker: "single" });
  assert.throws(() => finishPad(run, "define"), /empty/);
});
...
test("hibernating agent cannot append or finish — that is how we prevent the read", () => {
  let run = openRun({ charge: CHARGE, walker: "clone" });
  run = spawnClone(run, "define");
  run = spawnClone(run, "redefine");
  run = interrupt(run, "clone-define");
  ...
  run = appendPad(run, "define", "locked P_L");
  run = finishPad(run, "define");
  assert.throws(() => finishPad(run, "redefine"), /hibernat|empty/);
});
...
test("single walker may read its own finished prior pads", () => {
  let run = openRun({ charge: CHARGE, walker: "single" });
...
test("after mark, ingest returns finished copies and delete drops leftovers", () => {
  let run = fourFinished(openRun({ charge: CHARGE, walker: "single" }));
...
test("incomplete syndrome: three pads finished still cannot mark", () => {
  let run = openRun({ charge: CHARGE, walker: "single" });
  for (const id of ["define", "redefine", "explore"] as const) {
    run = appendPad(run, id, id);
    run = finishPad(run, id);
...
test("Zeno trap: thrashing interrupt before finish prevents the mark", () => {
  let run = openRun({ charge: CHARGE, walker: "clone" });
  run = spawnClone(run, "define");
  run = spawnClone(run, "explore");
  run = interrupt(run, "clone-define");
