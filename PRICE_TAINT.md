# Price’s Law as top variable — taint map
...
**Class B — name leftover:** “Price surface” for temp/top-k/top-p/steer. Different *job* (inference knobs). The *name* still comes from the mash. Do not silently keep it as if it were a second Price’s Law. Wait for meaning’s new name.
**Class C — shape bookkeeping:** \(T_{\mathrm{tail}}\times F\), √N tail share. May remain **shape** \(\hat{P}_L\), never the top variable, never viability.
## Repos
| Repo | Where | Class |
|---|---|---|
| **llmve-meaning** | `meaning/INSTRUCTION_SET.md` §4 “Viability under Price’s Law”; “Price-shaped working set”; causally-select. `OPERATOR_LOCKS.md` “Price vs Pareto”. `PATH_CHAIN_*` Price surface ≠ burn-away. | A+B — **being corrected** |
...
Attachments in this workspace (`ASSET_DENSITY_PL_LOCK_*`, `SENSOR_GAP_*`, `LLM Viability Equation Proof.md`, Logic-and-Reason “True vs Inverse Price’s Law” pools) are **historical**. Do not treat as SSOT.
## Do not
- Rewrite meaning here.
- Invent a replacement top variable.
- Invent an Ω close from this map, or rule out close from it.
