...
Old rule: only exclusive accounts (A ∧ B = ⊥).

This run: clone also when LRR names an **interesting resolving pathway** — a function-set that is **not** the current lane, **not** a synonym, **not** a specialization of the current account, and that has `necessary_because` + a residue **marker** for an **empty meet** still on the charge (a dump/sensor that could fill an open row).
...
- Start **low**: interesting-path fork requires **all** of: marker, `necessary_because`, empty meet with current lanes, not a synonym.
- Exclusive fight still forks (same gates).
...
1. Scout dumps/docs/papers for LLMVE **open rows** (not Holtzman). Name current lane.
2. LRR: if exclusive fight → clone. If interesting resolving pathway → clone. Else stay.
3. Each clone: one pathway. Parent notes only. No sibling list in the prompt (don’t bother attacking disk).
4. Parallel awake OK (H0). Cap still 10.
5. Meet when no more legal forks or SENS==10. `collate-hourglass`.
...
HOLD after board. Lattice default not rewritten. No ACL.
